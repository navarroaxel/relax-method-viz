...
export interface Mu0ScatterPoint {
  currentA: number;
  /** μ₀ at this point, in units of ×10⁻⁶ H/m — the guide's own quoted form. */
  mu0Micro: number;
  /** Propagated Δμ₀, same units as {@link mu0Micro}. */
  errorMicro: number;
}

interface Lab2ErrorScatterChartProps {
  points: Mu0ScatterPoint[];
  meanMu0Micro: number;
  xLabel: string;
  yLabel: string;
  pointLabel: string;
  ...
  formatSample: (point: Mu0ScatterPoint, index: number) => string;
}
...
function extentX(points: Mu0ScatterPoint[]): [number, number] {
...
function extentY(points: Mu0ScatterPoint[], meanMu0Micro: number): [number, number] {
  let lo = meanMu0Micro;
  let hi = meanMu0Micro;
  for (const p of points) {
    const upper = p.mu0Micro + p.errorMicro;
    const lower = p.mu0Micro - p.errorMicro;
...
/**
 * Scatter of μ₀ vs I with a vertical error bar (± Δμ₀) per point and a
 * horizontal line at the mean — the same layout as Lab 1's per-point error
 * chart, applied here to the §2.2 budget instead of the solenoid field.
 */
export function Lab2ErrorScatterChart({
  points,
  meanMu0Micro,
  xLabel,
  yLabel,
  ...
  const handleMove = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      const [yLo, yHi] = extentY(points, meanMu0Micro);
      let best = -1;
      let bestD = Infinity;
      points.forEach((p, k) => {
        const sx = MARGIN.left + ((p.currentA - xLo) / (xHi - xLo)) * plotW;
        const sy = MARGIN.top + plotH - ((p.mu0Micro - yLo) / (yHi - yLo)) * plotH;
    ...
    [meanMu0Micro, plotH, plotW, points],
  );
  ...
  useEffect(() => {
    ...
    // Mean μ₀, as a dashed reference line across the whole plot.
    ctx.strokeStyle = palette.mean;
    ctx.setLineDash([5, 4]);
    ctx.lineWidth = 1.4;
    ctx.beginPath();
    ctx.moveTo(sx(xLo), sy(meanMu0Micro));
    ctx.lineTo(sx(xHi), sy(meanMu0Micro));
    ctx.stroke();
    ctx.setLineDash([]);

    // Error bars: a vertical stem from μ₀−Δμ₀ to μ₀+Δμ₀, with a short
    ...
    for (const p of points) {
      ...
      const yUpper = sy(p.mu0Micro + p.errorMicro);
      const yLower = sy(p.mu0Micro - p.errorMicro);
      ctx.beginPath();
    ...
    for (const p of points) {
      ctx.beginPath();
      ctx.arc(sx(p.currentA), sy(p.mu0Micro), 3, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
    ...
    if (hoverIndex !== null) {
      ...
      if (p) {
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(sx(p.currentA), sy(p.mu0Micro), 6, 0, Math.PI * 2);
        ctx.stroke();
      }
    }
  }, [
