...
import { PRESETS_3D, apply, type Preset3DId } from "@/lib/presets3d";
...
describe.each(PRESET_IDS)("preset3d %s", (id) => {
  it.each(SIZES)(
    "has fixed voxels and all are in the interior [1,N-2]^3 (N=%i)",
    (N) => {
...
describe("preset3d-specific voltage checks", () => {
  ...
  it.each(SIZES)(
    "coax: has a grounded (0V) outer shell and +80V inner conductor (N=%i)",
    (N) => {
      const g = createGrid3D(N);
  ...
  it.each(SIZES)(
    "lightning: has +100000 cloud plate and a grounded (0V) rod/earth (N=%i)",
    (N) => {
      const g = createGrid3D(N);
  ...
  it.each(SIZES)(
    "faraday: has +80V source plate and grounded (0V) box (N=%i)",
    (N) => {
      const g = createGrid3D(N);
  ...
  it.each(SIZES)(
    "faraday: box interior is hollow (erased) not fixed (N=%i)",
    (N) => {
      const g = createGrid3D(N);
      PRESETS_3D.faraday.apply(g);
  ...
  it.each(SIZES)(
    "subconductors: has cells at V1 = sqrt(2/3)*500000 and a grounded plate (N=%i)",
    (N) => {
      const g = createGrid3D(N);
